...
function HostLibrary({ theme, host, layout, navigation }: PluginSurfaceProps) {
  ...
  const catalog = useQuery({
    queryKey: [host.id, "gortex", "catalog-browser-v2", query, filters.workspace, filters.project, filters.sort, offset],
    queryFn: () => listCatalog({ ...filters, query, offset, limit: repositoryPageSize }),
    ...
    <View style={{ width: "100%", maxWidth: 1440, alignSelf: "center", gap: 16, minWidth: 0 }}>
      <View style={{ flexDirection: "row", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
          {section === "repositories" && !repository && <Action title={catalog.isFetching ? "Refreshing…" : "Refresh"} icon="RefreshCw" theme={theme} disabled={catalog.isFetching || !queryCurrent} onPress={() => { void catalog.refetch(); }} />}
