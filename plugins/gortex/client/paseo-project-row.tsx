...
type Props = { theme: PluginSurfaceProps["theme"]; host: PluginSurfaceProps["host"]; repository: Repository; compact: boolean; trackingAvailable: boolean; onIndex: () => void; onGitChanged: () => void };
export function PaseoProjectRow({ theme, host, repository, compact, trackingAvailable, onIndex, onGitChanged }: Props) {
  const canIndex = repository.state === "untracked";
  const status = canIndex ? "Not indexed" : repository.state === "worktree" ? "Worktree · check view" : repository.state === "unsupported" ? "Can't verify" : "Folder unavailable";
  return <View style={{ borderTopWidth: 1, borderColor: theme.colors.border, padding: 12, gap: 10 }}>
    <View style={{ flexDirection: compact ? "column" : "row", alignItems: compact ? "stretch" : "center", gap: 12, minWidth: 0 }}>
      <View style={{ flex: 1, flexDirection: "row", alignItems: "center", gap: 12, minWidth: 0 }}>
        ...
        <View style={{ flex: 1, gap: 5, minWidth: 0 }}>
          ...
          <Text style={{ color: theme.colors.foregroundMuted, fontSize: 11, lineHeight: 17 }}>Paseo project · {repository.git === false ? "Plain folder, no Git" : "No dedicated Gortex tracking entry"}</Text>
        </View>
      </View>
    ...
    {repository.git === false && <GitInitControl host={host} theme={theme} path={repository.path} onChanged={onGitChanged} />}
