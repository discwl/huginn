...
export async function runProcess(command: string, args: readonly string[], cwd: string, options: { timeoutMs?: number; maxBytes?: number; env?: NodeJS.ProcessEnv; windowsVerbatimArguments?: boolean } = {}): Promise<string> {
  const env = options.env ?? process.env;
  const executable = await resolveHostExecutable(command, env);
  return new Promise((resolve, reject) => {
    const child = spawn(executable, [...args], { cwd, env, shell: false, windowsHide: true, windowsVerbatimArguments: options.windowsVerbatimArguments, stdio: ["ignore", "pipe", "pipe"] });
    const chunks: Buffer[] = [];
    let bytes = 0;
    let settled = false;
